...
- 添加新闻：在 `news` 数组顶部添加一条记录，照片放在 `public/images/news/`。新闻页按 `year` 自动归档。
- 添加论文：在 `publications` 数组顶部填写标题、作者、年份、会议和链接；检索和年份筛选自动更新。
- 添加教学、服务、奖项、教育经历：在对应数组添加记录。
...
可选的 `image`、`date`、`links` 不填时可省略。首页最多展示最近 5 条新闻、4 篇论文和 4 条教学经历，独立页面展示全部记录。Service 的 `href` 用来给会议／期刊名称加超链接；`reviewingAssistance` 用于“Help in Reviewing”，只在 Service 独立页面展示，不显示在首页。
