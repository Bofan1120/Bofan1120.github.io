...
可选的 `image`、`date`、`links` 不填时可省略。首页最多展示最近 5 条新闻和 4 条教学经历，论文只展示一作条目；独立页面展示全部记录。Service 的 `href` 用来给会议／期刊名称加超链接；`reviewingAssistance` 用于“Help in Reviewing”，只在 Service 独立页面展示，不显示在首页。
